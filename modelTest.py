...
# 2. KONFIGURATION
# ==========================================
# WICHTIG: Wenn du den Coral USB Stick nutzt, trage hier die Datei mit "_edgetpu" ein!
MODEL_PATH = "trainedDataCleanCode_edgetpu.tflite" 
...
# ==========================================
# 4. TENSORFLOW LITE INTERPRETER STARTEN
# ==========================================
print("Lade TFLite Modell mit Coral Edge TPU Delegate...")

# Wir probieren verschiedene Pfade aus, da sich das je nach OS-Version unterscheidet
delegate_paths = [
    'libedgetpu.so.1',
...
for path in delegate_paths:
    try:
        coral_delegate = tflite.load_delegate(path)
        print(f"Coral Delegate gefunden unter: {path}")
        break # Erfolgreich geladen, Schleife abbrechen
    except ValueError:
        pass # Pfad hat nicht funktioniert, nächsten probieren

if coral_delegate is None:
    print("FEHLER: Konnte den Coral Treiber 'libedgetpu.so.1' nicht laden.")
    print("Bitte prüfe, ob der Stick eingesteckt ist und die Rechte stimmen:")
    print("1. sudo usermod -aG plugdev $USER")
    print("2. sudo udevadm control --reload-rules && sudo udevadm trigger")
    sys.exit()

try:
    # Und übergeben ihn an den Interpreter
    ...
    print(f"Allgemeiner Fehler beim Laden des Modells: {e}")
    sys.exit()
...
try:
    picam2 = Picamera2(0) # Kamera 0 (ggf. auf 1 ändern, falls die falsche startet)
...
while True:
    ...
    # Debug-Fenster für die Maske (sehr nützlich zum Einstellen des Lichts!)
    ...
    # Debug-Fenster: Das sieht das CNN
    ...
    if confidence > MIN_CONFIDENCE:
        ...
        if label_str and label_str.lower() != "background":
            ...
            # Zeichne Bounding Box für den Buchstaben
            if box_coords:
    ...
    if detected_label == "Nichts gefunden":
        ...
        if corners is not None:
            # Zeichne die erkannte Ring-Fläche ein
            pts = corners.reshape((-1, 1, 2)).astype(np.int32)
            cv2.polylines(display_frame, [pts], True, (255, 165, 0), 3) # Orange
            ...
            if circle_status in ["H", "S", "U"]:
                ...
                color = (255, 165, 0) # Orange für Ring-Erkennung
                
                # Debug-Fenster für den flachen Ring
    ...
    # Abbruchbedingung: 1ms warten, bei 'q' beenden
