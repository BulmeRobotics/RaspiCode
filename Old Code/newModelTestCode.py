"""Standalone Test-Script for Raspberry Pi target detection and classification."""
...
# 1. CONFIGURATION
# ==========================================
# GEÄNDERT: Neue Modell- und Labelpfade
MODEL_PATH = "trainedWM.tflite"
...
# Camera Stream Visualization Settings (Zum Testen einfach auf True setzen!)
SHOW_STREAM_L = False
SHOW_STREAM_R = False
...
# ==========================================
# 4. CAMERA AI THREAD CLASS (TEST MODE)
# ==========================================
class CameraAIThread(threading.Thread):
    def __init__(self, cam_id, side_code, status_gpio_pin, show_stream=False):
        super().__init__()
        self.cam_id = cam_id
        self.side_code = side_code
        self.show_stream = show_stream

        # GEÄNDERT FÜR TESTMODUS: Kameras sind dauerhaft aktiv
        self.enabled = True
        self.running = True
        ...
        try:
            ...
            self.ready = True
            print(f"Cam {self.side_code} CPU Modell ({MODEL_PATH}) geladen.")
    ...
    def run(self):
        ...
        print(f"Thread {self.side_code} läuft im TEST-MODUS (Stream: {self.show_stream})")
        frame_count = 0
        ...
        while self.running:
            ...
            if self.show_stream:
                ...
                cv2.line(display_frame, tuple(coords["left_auswurf_top"]), tuple(coords["left_auswurf_bottom"]), (0, 0, 255), 2)
                cv2.line(display_frame, tuple(coords["right_auswurf_top"]), tuple(coords["right_auswurf_bottom"]), (0, 0, 255), 2)
            ...
            for c in valid_contours:
                ...
                if w_tmp < 60 or h_tmp < 60 or w_tmp > 360 or h_tmp > 360:
                    continue
                ...
                if confidence > MIN_CONFIDENCE_C:
                    ...
                    if label_str and label_str.lower() != "background":
                        if label_str.upper() == "C":
                            ...
                            print(f"[{self.side_code}] Kognitives Ziel erkannt (C). Confidence: {confidence:.3f}. Analysiere Ringe...")
                            color_result = self.evaluate_color_target(picam2, samples=15, delay=0.03)
                            if color_result:
                                # GEÄNDERT: Auffällige Ausgabe direkt im Terminal statt Serial
                                print("\n" + "="*55)
                                print(f"🧠 [{self.side_code}] KOGNITIVES ZIEL C + FARBE ERKANNT: {color_result}")
                                print("="*55 + "\n")
                                self.reset_logic()
                                time.sleep(1) # Kurze Pause nach Erkennung
                                continue
                            else:
                                print(f"[{self.side_code}] Farbauswertung lieferte kein klares Ergebnis.")
                        else:
            ...
            if self.show_stream:
                if detected_frame_label:
                    color = (0, 255, 0)
                    cv2.putText(display_frame, f"Erkannt: {detected_frame_label}", (10, cutoff_top_y + 30),
                    ...
                    cv2.putText(display_frame, "Suche...", (10, cutoff_top_y + 30),
            ...
            if detected_frame_label:
                ...
                # Confidence im Terminal ausgeben für Debugging
                print(f"[{self.side_code}] Frame {self.frame_counter}/10 - Erkannt: {detected_frame_label}")
            ...
            # --- TRANSMIT RESULT (TEST-AUSGABE) ---
            if self.frame_counter >= 10:
                counts = {"H": self.Counter_Harmed, "S": self.Counter_Safe, "U": self.Counter_Unharmed}
                cam_transmit = max(counts, key=counts.get)
                
                # GEÄNDERT: Fette Ausgabe ins Terminal
                print("\n" + "="*50)
                print(f"🏆 [{self.side_code}] FINALES ERGEBNIS: {cam_transmit}")
                print("="*50 + "\n")
                
                self.reset_logic()
                time.sleep(1) # Kurze Pause, damit der Log lesbar bleibt
...
# ==========================================
# 5. MAIN CONTROL LOOP (TEST RUNNER)
# ==========================================
if __name__ == "__main__":
    print("\n" + "="*50)
    print("STARTE ROBO-CUP KI TESTMODUS")
    print("Modell: trainedWM.tflite")
    print("Drücke STRG+C zum Beenden.")
    print("="*50 + "\n")

    cam_right = CameraAIThread(0, "R", ACTIVE_PIN_R, show_stream=SHOW_STREAM_R)
    cam_left = CameraAIThread(1, "L", ACTIVE_PIN_L, show_stream=SHOW_STREAM_L)
    ...
    try:
        while True:
            # Endlosschleife, damit das Hauptskript offen bleibt,
            # während die Kamera-Threads arbeiten.
            time.sleep(0.5)

    except KeyboardInterrupt:
        print("\nTestmodus wird beendet...")
        cam_left.running = False
        cam_right.running = False
        cam_left.join()
        cam_right.join()
        output_pin.off()
        print("Fertig.")